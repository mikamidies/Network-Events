<template lang="html">
  <div class="master">
    ...
    <div class="container">
      <div class="top">
        <div class="img">
          <span class="tag">{{ $store.state.translations["community.community1"] }} -
        ...
        <div class="info">
          ...
          <p>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
              <ellipse cx="10" cy="13.75" rx="5" ry="2.08333" stroke="#9A999B" stroke-width="1.2"
                stroke-linejoin="round" />
              <ellipse cx="10" cy="6.66663" rx="2.5" ry="2.5" stroke="#9A999B" stroke-width="1.2"
                stroke-linejoin="round" />
          ...
          <p class="security" :class="event?.public ? 'public' : 'private'">
            <svg v-if="event?.public" width="20" height="20" viewBox="0 0 20 20" fill="none"
              ...
              <path
                ...
                stroke="#00CD69" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
            ...
            <svg v-else width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                stroke="#EB5757" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
            {{
              event?.public
        ...
        <div class="messangers">
          <a target="_blank" :class="{ disabled: !event?.telegram_chat }" :href="event?.telegram_chat">
            <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                stroke="#1878F3" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            {{ $store.state.translations["community.tg_chat"] }}
          </a>
          <button class="presence nopresence" v-if="event?.is_member" @click="toLeave(event?.uuid)">
          ...
          <button class="presence" @click="toMember(event?.uuid)" v-else>
        ...
        <div class="tabs">
          ...
          <button v-if="memberStatus" @click="tabHandle = 'participants'"
          ...
          <button @click="tabHandle = 'contact'" :class="{ active: tabHandle == 'contact' }">
            {{ $store.state.translations["event.for_contact"] }}
          </button>
        </div>
        <div class="body">
          <div v-show="tabHandle == tabItem.key" v-for="(tabItem, index) in tabList" :key="index" :class="tabItem.class"
            :propObj="tabItem.propItem">
            <component :is="tabItem.name" :event="event" @tabChange="tabChange" :members="members" :coords="coords"
              :memberStatus="memberStatus" @getData="__GET_MEMBERS" :totalPage="totalPage" />
          </div>
        </div>
      </div>
    </div>
      ...
      try {
        const data = await communityApi.getMembers({
          ...
          payload: {
            params: {
              ...this.$route.query,
              page_size: this.$route.query?.page_size || 40,
            },
          },
        });
